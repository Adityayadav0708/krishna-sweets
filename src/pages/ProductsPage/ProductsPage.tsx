...
const categories: Array<Product['category']> = ['Signature', 'Bengali', 'Ladoo', 'Festive'];
...
const isPriceRange = (value: string | null): value is PriceRange =>
  priceRanges.some((range) => range.value === value);

const isSortOption = (value: string | null): value is SortOption =>
  sortOptions.some((option) => option.value === value);

const matchesPrice = (product: Product, range: PriceRange) => {
  if (range === 'under-400') return product.price < 400;
  if (range === '400-600') return product.price >= 400 && product.price <= 600;
  if (range === 'above-600') return product.price > 600;
  return true;
};
...
export function ProductsPage() {
  ...
  const selectedCategory: Product['category'] | 'all' = categories.includes(rawCategory as Product['category'])
    ? rawCategory as Product['category']
    : 'all';
  const selectedPrice: PriceRange = isPriceRange(rawPrice) ? rawPrice : 'all';
  const selectedSort: SortOption = isSortOption(rawSort) ? rawSort : 'featured';
    ...
    <div className={styles.filterPanel}>
      ...
      <fieldset>
        ...
        {priceRanges.map((range) => (
          <button
            type="button"
            className={selectedPrice === range.value ? styles.activeFilter : ''}
  ...
  return (
    <div id="top">
      ...
      <main>
        <section className={styles.hero}>
          <Container className={styles.heroInner}>
            <SectionHeading
              ...
              title="Choose sweets for every celebration."
              body="Search, filter and sort our handcrafted favourites. Your selection stays in the URL, ready to share or revisit."
            />
            <form className={styles.searchBar} role="search" onSubmit={(event) => event.preventDefault()}>
              ...
              <input
                ...
                placeholder="Search kaju, ladoo, Diwali..."
        ...
        <div
          className={`${styles.mobileFilterBackdrop} ${filtersOpen ? styles.mobileFilterBackdropOpen : ''}`}
