...
export function ProductDetailPage() {
  ...
  const [selectedSize, setSelectedSize] = useState<ProductSize>('500g');
              ...
              <div className={styles.gallery} aria-label={`${product.name} image gallery`}>
                ...
                <div className={styles.thumbnails}>
                  {product.gallery.map((image, index) => (
                    <button
                      type="button"
                      className={selectedImage === index ? styles.activeThumb : ''}
                ...
                <div className={styles.sizePicker}>
                  ...
                  <div>
                    {product.sizes.map((size) => (
                      <button
                        type="button"
                        className={selectedSize === size.label ? styles.selectedSize : ''}
          ...
          <section className={styles.relatedSection}>
            <Container>
              <SectionHeading
                ...
                title="Related handcrafted favourites."
                body="Chosen from the same collection and our most-loved daily sweets."
                align="center"
              />
              <div className={styles.relatedGrid}>
                {relatedProducts.map((entry) => (
                  <ProductCard
                    product={entry}
                    added={items.some((item) => item.productId === entry.id)}
                    onAdd={addItem}
                    key={entry.id}
                  />
                ))}
